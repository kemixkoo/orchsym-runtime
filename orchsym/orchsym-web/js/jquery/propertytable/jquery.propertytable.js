...
(function (root, factory) {
    ...
    var promptForNewControllerService = function (gridContainer, grid, item, serviceType, bundle, configurationOptions) {
        $.ajax({
            type: 'GET',
            url: apiHost + "/nifi-api/flow/controller-service-types",
            data: {
            ...
            if (options.length === 0) {
                ...
                var create = function () {
                    ...
                    var uri = apiHost + "/nifi-api/controller/controller-services";
                    if (nfCommon.isDefinedAndNotNull(groupId)) {
                        uri = apiHost + "/nifi-api/process-groups/" + encodeURIComponent(groupId) + '/controller-services';
                    }
    ...
    var initPropertiesTable = function (table, options) {
        ...
        var goToControllerService = function (property) {
            $.ajax({
                type: 'GET',
                url: apiHost + "/nifi-api/controller-services/" + encodeURIComponent(property.value),
                dataType: 'json'
