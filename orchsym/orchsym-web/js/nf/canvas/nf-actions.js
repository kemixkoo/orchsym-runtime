...
(function (root, factory) {
    ...
    var initDeleteModel = function() {
           ...
           var $html = '<div id="deleteModal" class="hidden"  style="z-index: 1301;height:150px;width:320px;position:absolute;left:505.5px;top:319px;background-color:#FFFFFF;">' +
                            ...
                            '<div class="dialog-buttons"><div id="deleteOk" class="button ok" style="background: rgb(114, 142, 155); color: rgb(255, 255, 255);">'+nf._.msg('nf-actions.Ok')+'</div><div id="deleteCancel" class="button cancel" style="background: rgb(227, 232, 235); color: rgb(0, 72, 73);" >'+nf._.msg('nf-actions.Cancel')+'</div></div>'+
                        '</div>';
            $("body").append($mengban)
            $("body").append($html)
    ...
    var nfActions = {
        ...
        start: function(selection){
            ...
            nfSnippet.create(snippet).done(function (response) {
                $.ajax({
                    type: 'GET',
                    url: apiHost + "/nifi-api/validation/snippet/"+response.snippet.id,
                    dataType: 'json'
                }).done(function (response) {
                    // try{
        ...
        showUsage: function (selection) {
            if (selection.size() === 1 && nfCanvasUtils.isProcessor(selection)) {
                var selectionData = selection.datum();
                nfShell.showPage(apiHost + "/nifi-docs/documentation?" + $.param({
        ...
        'delete': function (selection) {
            ...
            if (needPrompt) {
                ...
                if(selection.size() === 1) {
                    $.ajax({
                        type: 'GET',
                        url: apiHost + "/nifi-api/stats/delete/"+selectionData.id,
                        dataType: "text"
                    }).done(function (response) {
                    ...
                    nfSnippet.create(snippet).done(function (response) {
                        $.ajax({
                            type: 'GET',
                            url: apiHost + "/nifi-api/stats/delete/"+response.snippet.id,
                            dataType: 'text'
                        }).done(function (response) {
                            $('#deleteModal').removeClass('hidden')
                            $('#deleteMengBan').removeClass('hidden')
        ...
        emptyQueue: function (selection) {
            ...
            nfDialog.showYesNoDialog({
                ...
                yesHandler: function () {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: apiHost + "/nifi-api/flowfile-queues/" + encodeURIComponent(connection.id) + '/drop-requests',
                        dataType: 'json',
                        contentType: 'application/json'
                    }).done(function (response) {
